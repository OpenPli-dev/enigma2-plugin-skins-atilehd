...
class AtileHDEditScreens(Screen):
    skin = """
  <screen name="AtileHDEditScreens" position="0,0" size="1280,720" title="AtileHD EditScreens" backgroundColor="transparent" flags="wfNoBorder">
...
"""
    ...
    def __init__(self, session, ScreenFile = ''):
        ...
        if self.myScreenName == None:
            myTitle=_("AtileHD %s - EditScreens") %  AtileHDInfo
        else:
            if self.NumberOfScreens == 1:
                myTitle=_("AtileHD %s - Edit %s screen") %  (AtileHDInfo,self.myScreenName)
            else:
                myTitle=_("AtileHD %s - Edit %s screen (1/%d)") %  (AtileHDInfo,self.myScreenName,self.NumberOfScreens)
    ...
    def getPicFileNameWithPath(self, filename):
        if path.isfile("%sAtileHDpics/%s" % (SkinPath,filename) ):
            return "%sAtileHDpics/%s" % (SkinPath,filename)
        else:
