...
"""
    ...
    def __init__(self, session, args = 0):
        ...
        myTitle=_("AtileHD Setup %s") % AtileHDInfo
    ...
    def keyOk(self):
        if self["config"].isChanged() or self.updateEntries == True:
            printDEBUG("[AtileHD:keyOk] self.myAtileHD_font.value=" + self.myAtileHD_font.value)
            printDEBUG("[AtileHD:keyOk] self.myAtileHD_style.value=" + self.myAtileHD_style.value)
            printDEBUG("[AtileHD:keyOk] self.myAtileHD_windowstyle.value=" + self.myAtileHD_windowstyle.value)
            printDEBUG("[AtileHD:keyOk] self.myAtileHD_bar.value=" + self.myAtileHD_bar.value)
            ...
            if path.exists("allBars/" + self.myAtileHD_bar.value):
                ...
                printDEBUG("[AtileHD:keyOk]cp -fr %s %s" % (sourcePath,destPath))
...
##### userBARs #####        
    def getCurrentBar(self):
        ...
        printDEBUG("[AtileHD:getCurrentBar] myfile='%s'" % myfile)
        if not path.exists(myfile):
    ...
    def getPossibleBars(self):
        #printDEBUG("[AtileHD:getPossibleBars] >>>")
        bar_list = []
        ...
        for f in sorted(listdir(self.skin_base_dir + "allBars/"), key=str.lower):
            ...
            printDEBUG("[AtileHD:getPossibleBars] f='%s'" % f)
...
"""
    ...
    def __init__(self, session):
        ...
        myTitle=_("AtileHD %s - additional screens") %  AtileHDInfo
        self.setTitle(myTitle)
        try:
            self["title"]=StaticText(myTitle)
        except:
        ...
        if path.exists("%sAtileHDpics/install.png" % SkinPath):
            ...
            self.enabled_pic = LoadPixmap(cached=True, path=resolveFilename(SCOPE_PLUGINS, "Extensions/AtileHD/pic/install.png"))
        ...
        if self.currentSkin == "infinityHD-nbox-tux-full" and isPreview < 2:
            ...
            if path.exists("%sAtileHDpics/install.png" % SkinPath):
                ...
                self.disabled_pic = LoadPixmap(cached=True, path=resolveFilename(SCOPE_PLUGINS, "Extensions/AtileHD/pic/remove.png"))
