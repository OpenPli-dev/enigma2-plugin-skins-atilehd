...
def Plugins(**kwargs):
    return [PluginDescriptor(name=_("AtileHD Setup"), description=_("Personalize your Skin"), where = PluginDescriptor.WHERE_MENU, fnc=menu)]

def menu(menuid, **kwargs):
    for line in open("/etc/enigma2/settings"):
     if "config.skin.primary_skin=AtileHD/skin.xml" in line:
      if menuid == "mainmenu":
        return [(_("Setup -") + " " + CurrentSkinName, main, "AtileHD_Menu", 40)]
    return []
