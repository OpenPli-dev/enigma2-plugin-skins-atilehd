...
class AtileHDconsole(Screen):
    ...
    def __init__(self, session, title = "AtileHDconsole", cmdlist = None, finishedCallback = None, closeOnSuccess = False):
        ...
        if selecteditem is not None:
            for opcja in self.myList:
                if opcja[0] == selecteditem:
                    ...
                    if opcja[1] == "CONSOLE":
                        self.session.openWithCallback(self.endrun ,AtileHDconsole, title = "%s" % selecteditem, cmdlist = [ ('%s' %  self.SkryptOpcji) ])
                    if opcja[1] == "YESNO":
