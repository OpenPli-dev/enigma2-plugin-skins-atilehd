				...
				if info.getInfoObject(iServiceInformation.sCAIDs):
					...
					if ecm_info:
						...
						if self.type == self.ALL:
							if source == "emu":
								textvalue = "(Prov: %s, Caid: %s)" % (self.systemTxtCaids.get(caid[:2]), prov, caid)
							#new oscam ecm.info with port parametr
							elif server == "" and port == "": 
								textvalue = "Prov: %s, Caid: %s, Reader: %s, %s %s %s - %s" % (prov, caid, reader, protocol, server, port, ecm_time.replace('msec','ms'))
							elif server == "" and port == "":  
								textvalue = "Prov: %s, Caid: %s, Reader: %s, %s (%s) - %s" % (prov, caid, reader, protocol, server, ecm_time.replace('msec','ms'))
							elif server == "" and port == "": 
								textvalue = "Prov: %s, Caid: %s, Reader: %s, %s (local) - %s" % (prov, caid, reader, protocol, ecm_time.replace('msec','ms'))
							elif server == "" and port == "": 
								textvalue = "Prov: %s, Caid: %s, %s - %s" % (prov, caid, protocol, ecm_time.replace('msec','ms'))
							elif server == "" and port == "": 
								textvalue = "Prov: %s, Caid: %s - %s" % (prov, caid, ecm_time.replace('msec','ms'))
							else:
								try:
									textvalue = "Prov: %s, Caid: %s, %s (%s:%s) - %s" % (prov, caid, protocol, server, port, ecm_time.replace('msec','ms'))
								except:
									pass
						if self.type == self.SHORT:
							if source == "emu":
								...
								try:
									textvalue = "Prov: %s, Caid: %s, %s:%s - %s" % (prov, caid, server, port, ecm_time.replace('msec','ms'))
								except:
									pass
					else:
